<script>
export default {
  name: 'Card'
}
</script>

<template> 
  <!-- Cards de projetos -->
  <h2 id="projetos" style="text-align: center; color: #00CFFF; text-shadow: 0 0 8px #00CFFF;">
    Meus Projetos
  </h2>
  <p style="text-align: center; margin-bottom: 30px;">
    Aqui estão alguns dos meus projetos recentes, desenvolvidos com dedicação e criatividade.
  </p>

  <div class="card-group">
    <!-- Miauh Timer -->
    <div class="card">
      <img src="@/assets/img/projetos/Miauh.png" class="card-img-top" alt="Miauh Timer">
      <div class="card-body">
        <h5 class="card-title">Miauh Timer 🐾</h5>
        <p class="card-text">
          Aplicativo temático inspirado em gatos, desenvolvido para cálculo de diferença de tempo entre duas datas/horas. Traz uma experiência divertida e funcional, com identidade visual personalizada.
        </p>
        <div class="card-footer">
          <span class="lang-text lang-flutter">Linguagem: Android Nativo (Java)</span>
          <router-link class="card-btn" to="/miauh">Ver Mais</router-link>
        </div>
        <div class="progress">
          <div class="progress-bar" style="width: 100%"></div>
        </div>
        <p class="card-text"><small>Última atualização há 3 mins</small></p>
      </div>
    </div>

    <!-- Chat Aly -->
    <div class="card">
      <img src="@/assets/img/projetos/Aly.png" class="card-img-top" alt="Chat Aly">
      <div class="card-body">
        <h5 class="card-title">Chat Aly 🤖</h5>
        <p class="card-text">
          Assistente virtual em forma de chat interativo, que combina mensagens, chamadas e até histórias interativas com escolhas do usuário. Um projeto que une comunicação e entretenimento de forma criativa.
        </p>
        <div class="card-footer">
          <span class="lang-text lang-flutter">Linguagem: Flutter (Dart)</span>
          <router-link class="card-btn" to="/aly">Ver Mais</router-link>
        </div>
        <div class="progress">
          <div class="progress-bar" style="width: 100%"></div>
        </div>
        <p class="card-text"><small>Última atualização há 3 mins</small></p>
      </div>
    </div>

    <!-- FluxoApp -->
    <div class="card">
      <img src="@/assets/img/projetos/Fluxo.png" class="card-img-top" alt="FluxoApp">
      <div class="card-body">
        <h5 class="card-title">FluxoApp 🚀</h5>
        <p class="card-text">
          Aplicativo de controle financeiro pessoal, com registro de gastos, gráficos dinâmicos, resumo de períodos e exportação em PDF. Focado em ajudar o usuário a equilibrar finanças de forma prática e visual.
        </p>
        <div class="card-footer">
          <span class="lang-text lang-flutter">Linguagem: Flutter (Dart)</span>
          <router-link class="card-btn" to="/fluxo">Ver Mais</router-link>
        </div>
        <div class="progress">
          <div class="progress-bar" style="width: 100%"></div>
        </div>
        <p class="card-text"><small>Última atualização há 3 mins</small></p>
      </div>
    </div>
  </div>

  <br>

  <div class="card-group">
    <!-- Guardião de Senhas -->
    <div class="card">
      <img src="@/assets/img/projetos/Guardiao.png" class="card-img-top" alt="Guardião de Senhas">
      <div class="card-body">
        <h5 class="card-title">Guardião de Senhas 🔐</h5>
        <p class="card-text">
          Aplicativo focado em armazenamento seguro de senhas. Permite organizar credenciais em pastas, proteger dados com criptografia e biometria, além de oferecer interface moderna no estilo Dark Tech.
        </p>
        <div class="card-footer">
          <span class="lang-text lang-flutter">Linguagem: Flutter (Dart)</span>
          <router-link class="card-btn" to="/guardiao">Ver Mais</router-link>
        </div>
        <div class="progress">
          <div class="progress-bar" style="width: 75%"></div>
        </div>
        <p class="card-text"><small>Última atualização há 3 mins</small></p>
      </div>
    </div>

    <!-- Neon Code Café -->
    <div class="card">
      <img src="@/assets/img/projetos/Neon.png" class="card-img-top" alt="Neon Code Cafe">
      <div class="card-body">
        <h5 class="card-title">Neon Code Cafe ☕</h5>
        <p class="card-text">
          Website institucional com tema futurista, apresentando um cardápio criativo inspirado no mundo da programação. Layout neon, responsivo e envolvente, com elementos visuais modernos e imersivos.
        </p>
        <div class="card-footer">
          <span class="lang-text lang-flutter">Linguagem: Flutter (Dart)</span>
          <router-link class="card-btn" to="/neon">Ver Mais</router-link>
        </div>
        <div class="progress">
          <div class="progress-bar" style="width: 100%"></div>
        </div>
        <p class="card-text"><small>Última atualização há 3 mins</small></p>
      </div>
    </div>

    <!-- Costura Certa (🚧 em progresso) -->
    <div class="card">
      <img src="@/assets/img/projetos/Costura.png" class="card-img-top" alt="Costura Certa">
      <div class="card-body">
        <h5 class="card-title">Costura Certa ✂️</h5>
        <p class="card-text">
          Aplicativo ainda em desenvolvimento voltado para gestão de pedidos de costura, permitindo cadastrar medidas, acompanhar pedidos e controlar parte financeira. Interface simples, organizada e com recursos de backup.
        </p>

        <div class="card-footer">
          <span class="lang-text lang-flutter">Linguagem: Flutter (Dart)</span>
          <router-link class="card-btn" to="/costura">Ver Mais</router-link>
        </div>

        <!-- 🚧 Em progresso -->
        <div class="status-progresso">
          <span class="emoji">🚧</span>
          <span class="texto-status">Em progresso...</span>
        </div>

        <div class="progress">
          <div class="progress-bar progress-animada" style="width: 10%"></div>
        </div>
        <p class="card-text"><small>Última atualização há 3 mins</small></p>
      </div>
    </div>
  </div>
  <br><br>
</template>

<style scoped>
.lang-flutter { color: #00CFFF; text-shadow: 0 0 6px #00CFFF; }

* { font-family: cursive; scroll-behavior: smooth; }

.progress {
  background: #c0c0c0ff;
  border-radius: 8px;
  height: 20px;
  overflow: hidden;
}
.progress-bar {
  background: linear-gradient(90deg, #00CFFF, #39FF14);
  height: 100%;
  transition: width 1.5s ease;
}
.progress-animada {
  animation: barra-movimento 2s infinite linear;
}

@keyframes barra-movimento {
  0% { background-position: 0 0; }
  100% { background-position: 40px 0; }
}

.status-progresso {
  display: flex;
  align-items: center;
  gap: 8px;
  font-weight: bold;
  color: #f5c518;
  font-size: 1rem;
  margin: 8px 0;
  animation: pulse 1.5s infinite;
}
@keyframes pulse {
  0%, 100% { opacity: 1; }
  50% { opacity: 0.6; }
}

.card-img-top {
  width: 250px;
  display: block;
  margin: 0 auto 15px auto;
  border-radius: 9px;
}
.card {
  background: #fff;
  border: 1px solid #222;
  border-radius: 12px;
  padding: 20px;
  width: 280px;
  box-shadow: 0 0 12px rgba(0, 207, 255, 0.2);
  transition: transform 0.2s ease, box-shadow 0.2s ease;
  color: #181717ff;
  margin: 20px auto;
}
.card:hover {
  transform: translateY(-6px);
  box-shadow: 0 0 20px rgba(0, 207, 255, 0.5);
}
.card-title {
  font-size: 1.3rem;
  color: #00CFFF;
  text-shadow: 0 0 8px #00CFFF;
}
.card-btn {
  background: linear-gradient(90deg, #00CFFF, #1495ff);
  border: none;
  padding: 10px 18px;
  border-radius: 8px;
  font-weight: bold;
  cursor: pointer;
  color: #0d0d0d;
  transition: 0.3s;
}
.card-btn:hover {
  filter: brightness(1.2);
  box-shadow: 0 0 10px #00CFFF;
}
.card-footer {
  display: flex;
  align-items: center;
  gap: 12px;
  margin-bottom: 10px;
}
</style>
